...
const NavbarItemRight = profile => (
  ...
  <Nav pullRight>
    <NavItem href="#">
      <svg alt="you have to unread notification" aria-hidden="true" className="octicon octicon-bell" height="16" version="1.1" viewBox="0 0 14 16" width="14">
    ...
    >
      {Object.keys(profile.dropDownPlusIconVal).map(key => (
        <MenuItem key={key}>
          {profile.dropDownPlusIconVal[key]}
        </MenuItem>
      ))}
    </NavDropdown>
    ...
    <NavDropdown
      eventKey={7}
      title={
        <img alt="" className="avatar float-left mr-1" src={profile.profile.iconImage} height="20" width="20" />}
      id="basic-nav-dropdown"
    >
      {Object.keys(profile.dropDownLogVal).map(key => (
        <MenuItem key={key}>
          {profile.dropDownLogVal[key]}
          {(profile.dropDownLogVal[key] === 'Signed in as' ? profile.profile.login : '')}      
        </MenuItem>
      ))}
    </NavDropdown>
  </Nav>
